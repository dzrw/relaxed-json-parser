/* rj.js v0.1.0 - a parser which recognizes a superset of JSON
...
// Augment the Unicode category block ranges with general category ranges 
// distinguished by their first letter (Z, P, S, L, etc.)
/*
    (function(uc) {
        var p0, groups = {};
        ...
        for ( var p in uc ) {
            p0 = p[0];
            groups[p0] = groups[p0]
                ? uc[p].substring(0, uc[p].length-1) + groups[p0].substring(1)
                : uc[p];
        }
...
// dialect supported by JavaScript.
        ...
        if (patternOrRegExp instanceof RegExp) {
            flags = "";
            flags += patternOrRegExp.global ? "g" : "";
            flags += patternOrRegExp.ignoreCase ? "i" : "";
            flags += patternOrRegExp.multiline ? "m" : "";
            patternOrRegExp = patternOrRegExp.source;
        }

        pattern = patternOrRegExp.replace(/\\p\{(..?)\}/g, function (match, group) {
...
// as defined in ECMA-262 7.6.  This expression is not immediately executable
// by RegExp and must be decoded first.
        
        var unicodeLetter = '\\p{Lu}|\\p{Ll}|\\p{Lt}|\\p{Lm}|\\p{Lo}|\\p{Nl}', // '[a-zA-Z]',
            unicodeCombiningMark = '\\p{Mn}|\\p{Mc}',
            unicodeDigit = '\\p{Nd}', // '[0-9]',
            unicodeConnectorPunctuation = '\\p{Pc}',
            identifierStart = ['[\\$_]', unicodeLetter].join('|'),
            identifierPart = [identifierStart, unicodeCombiningMark, unicodeDigit, unicodeConnectorPunctuation].join('|'),
            //identifierPart = [identifierStart, unicodeDigit].join('|'),
